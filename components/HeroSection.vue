<template>
  <section id="hero" class="relative w-full min-h-screen lg:h-screen flex items-center justify-center bg-gray-900 overflow-hidden">    
    ...
    >
      <div v-if="showSuccessToast" class="fixed top-24 left-4 right-4 sm:left-auto sm:right-5 z-[100] w-auto sm:w-full max-w-sm overflow-hidden bg-white dark:bg-[#121212] rounded-lg shadow-2xl border border-green-500/30">
        ...
        <div class="w-full bg-gray-200 dark:bg-gray-700 h-1">
          <div class="bg-green-500 h-1 transition-all duration-100 ease-linear" :style="{ width: toastProgress + '%' }"></div>
        </div>
      </div>
    </transition>
    ...
    <transition
      ...
      leave-to-class="opacity-0"
    >
      <div v-if="showErrorToast" class="fixed top-24 left-4 right-4 sm:left-auto sm:right-5 z-[100] w-auto sm:w-full max-w-sm overflow-hidden bg-white dark:bg-[#121212] rounded-lg shadow-2xl border border-red-500/30">
        <div class="p-4 flex items-start">
          <div class="flex-shrink-0">
          ...
          <div class="ml-3 w-0 flex-1 pt-0.5">
            <p class="text-sm font-medium text-gray-900 dark:text-white">Erro no Envio!</p>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Não foi possível enviar sua solicitação. Tente novamente ou use nossos canais de contato.</p>
          </div>
          <div class="ml-4 flex-shrink-0 flex">
            <button @click="closeErrorToast" class="rounded-md inline-flex text-gray-400 hover:text-gray-500 focus:outline-none">
              <span class="sr-only">Fechar</span>
              <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
              </svg>
        ...
        <div class="w-full bg-gray-200 dark:bg-gray-700 h-1">
          <div 
            class="bg-red-500 h-1 transition-all duration-100 ease-linear" :style="{ width: errorToastProgress + '%' }"
          ></div>
        </div>
      </div>
    </transition>
...
// --- CONTROLE DO TOAST (ERRO) ---
const showErrorToast = ref(false);
const errorToastProgress = ref(100);
let errorToastInterval = null;
const ERROR_DURATION = 5000;

const triggerErrorToast = () => {
  showErrorToast.value = true;
  errorToastProgress.value = 100;
  
  if (errorToastInterval) clearInterval(errorToastInterval);

  const step = 50;
  errorToastInterval = setInterval(() => {
    errorToastProgress.value -= (step / ERROR_DURATION) * 100;
    
    if (errorToastProgress.value <= 0) {
      closeErrorToast();
    }
  }, step);
};
...
const submitForm = () => {
  ...
  if (isNameValid && isEmailValid && isPhoneValid) {
    ...
    const templateParams = {
      name: form.name,
      store: form.store || 'Não informado',
      email: form.email,
      phone: form.phone
    };

    const SERVICE_ID = 'INSIRA_O_SERVICE_ID_AQUI'; 
    const TEMPLATE_ID = 'INSIRA_O_TEMPLATE_ID_AQUI'; 
    const PUBLIC_KEY = 'INSIRA_A_PUBLIC_KEY_AQUI'; 
